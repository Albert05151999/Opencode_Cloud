...
from shared_libs.workspace_paths import (
    WorkspacePathError,
    WorkspaceLayout,
    validate_identifier,
...
class WorkspaceManager:
    ...
    def _set_runtime_ownership(self, *paths: Path) -> None:
        ...
        for path in paths:
            try:
                current = path.stat()
                if current.st_uid != self.runtime_uid or current.st_gid != gid:
                    os.chown(path, self.runtime_uid, gid)
                if stat.S_IMODE(current.st_mode) != 0o770:
                    path.chmod(0o770)
    ...
    def resolve_user_path(
        ...
        *,
        ...
        if create_parent and parts:
            ...
            while current != scope:
                self._set_runtime_ownership(
                    self._resolve_contained(current, scope_real)
                )
