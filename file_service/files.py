...
from shared_libs.workspace_paths import WorkspacePathError, normalize_relative_parts
from shared_libs.logging import bind_request
...
class FileService:
        ...
        async with lock:
            ...
            try:
                ...
                used = await asyncio.to_thread(
                    self._tree_size,
                    self.workspaces.ensure_user_layout(agent_id, username).workspace,
                )
                fd = os.open(
                    temp_name,
                ...
                while chunk := await upload.read(1024 * 1024):
                    ...
                    if (
                        used - old_size + size
                        > self.config.max_user_workspace_gb * 1024**3
                    ):
                ...
                os.replace(
                    temp_name, filename, src_dir_fd=parent_fd, dst_dir_fd=parent_fd
                )
            except WorkspacePathError as exc:
                raise HTTPException(400, str(exc)) from exc
            finally:
    ...
    def _walk_directory(
        self, scope: Path, parts: tuple[str, ...], *, create: bool
    ) -> int:
...
def create_files_router(
    ...
    async def upload(
        ...
        coordination = getattr(
            getattr(request.scope.get("app"), "state", None), "file_coordination", None
        )
        if coordination:
            async with coordination.guard(agent_id):
                await coordination.admission(agent_id, username, upload=True)
                return await service.upload(
                    agent_id, username, file, relative_path, session_id
                )
    ...
    async def download(
        ...
        try:
            ...
            if coordination:
                async with coordination.guard(agent_id):
                    await coordination.admission(agent_id, username)
                    handle, filename, size = service.open_download(
                        agent_id, username, path, session_id
                    )
            else:
                handle, filename, size = service.open_download(
                    agent_id, username, path, session_id
    ...
    async def list_files(
        ...
        try:
            ...
            return {
                "entries": service.list_entries(agent_id, username, path, session_id)
            }
        except FileNotFoundError as exc:
