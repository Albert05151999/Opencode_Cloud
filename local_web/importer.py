...
from app.management import fail, redact, validate_model
...
class Importer:
    ...
    def discover(self, directory=None, file=None):
        config_home = Path(os.environ.get('XDG_CONFIG_HOME', str(self.home / '.config')))
        paths = [config_home / 'opencode' / n for n in ('opencode.json', 'opencode.jsonc')]
        for env in ('OPENCODE_CONFIG', 'OPENCODE_CONFIG_DIR'):
            value = os.environ.get(env)
            if value:
                p = Path(value).expanduser()
                paths.extend([p] if env == 'OPENCODE_CONFIG' else [p / n for n in ('opencode.json', 'opencode.jsonc')])
        if directory:
            p = Path(directory).expanduser().resolve()
            paths.extend(p / n for n in ('opencode.json', 'opencode.jsonc', '.opencode/opencode.json', '.opencode/opencode.jsonc'))
        if file:
        ...
        for p in dict.fromkeys(paths):
            if str(p).startswith('\\\\') or not p.is_file():
                continue
            resolved = p.resolve()
            key = next((k for k, v in self.sources.items() if v == resolved), None) or uuid.uuid4().hex
    ...
    def resolve_value(self, value, parent):
        ...
        def replace(match):
            ...
            if not p.is_absolute():
                p = parent / p
            p = p.resolve()
            if not (p.is_relative_to(self.home) or p.is_relative_to(parent)) or str(p).startswith('\\\\'):
                fail('Referenced file is outside the chosen configuration directory and user home')
    ...
    def preview(self, source_id):
        ...
        raw = {k: v for k, v in raw.items() if k in {'provider', 'model', 'small_model'}}
        config = self.resolve_value(raw, source.parent)
        auth = {}
        data_home = Path(os.environ.get('XDG_DATA_HOME', str(self.home / '.local/share')))
        ...
        for pid, provider in config.get('provider', {}).items():
            try:
                ...
                if unsupported:
                    fail('Unsupported provider options: ' + ', '.join(sorted(unsupported)))
                credential = auth.get(pid, {})
                key = options.get('apiKey') or (credential.get('key') if credential.get('type') == 'api' else None)
                if not key:
                ...
                if url.hostname in {'localhost', '127.0.0.1', '::1', '0.0.0.0'}:
                    fail('Local model URL is not reachable by the server; change baseURL before importing')
                for mid, model in provider.get('models', {}).items():
                    supported = {'id', 'name', 'limit', 'options', 'temperature', 'top_p', 'max_tokens'}
                    if set(model) - supported:
                        fail('Unsupported model fields: ' + ', '.join(sorted(set(model) - supported)))
                    if set(model.get('limit', {})) - {'context', 'output'}:
                        fail('Unsupported model limit fields; retain context and output only')
                    model_id = re.sub('[^a-zA-Z0-9_-]', '-', pid + '-' + mid)[:64]
                    entry = {'id': model_id, 'name': model.get('name', mid), 'provider': adapter,
                        'upstream_model': model.get('id', mid), 'base_url': base, 'api_key': key,
                        'headers': options.get('headers', {}), 'parameters': dict(model.get('options', {})), 'enabled': True}
                ...
                errors.append({'provider': pid, 'error': exc.detail if isinstance(exc, HTTPException) else 'Invalid provider structure'})
        if len({m['id'] for m in items}) != len(items):
            errors.append({'error': 'Imported model IDs collide; rename provider or model entries'})
        if not items:
        ...
        self.previews[preview_id] = {'models': items, 'errors': errors, 'expires': time.time() + 900}
        return {'preview_id': preview_id, 'models': redact(items), 'errors': errors,
                'model': config.get('model'), 'small_model': config.get('small_model'),
