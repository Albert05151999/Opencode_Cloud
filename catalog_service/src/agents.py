...
from catalog_service.src.management import identifier
...
class AgentCatalog:
    ...
    def load(self, agent_id: str) -> AgentDefinition:
        try:
            ...
            raise AgentDefinitionError(
                f"unknown or invalid Agent {agent_id!r}: {exc}"
            ) from exc
        ...
        try:
            if parser.read(path / "agent.cfg", encoding="utf-8") != [
                str(path / "agent.cfg")
            ]:
            ...
            allowed = tuple(
                item.strip()
                for item in parser.get("models", "allowed").split(",")
                if item.strip()
            )
        ...
        if (
            idle <= 0
            or resources.cpu_limit <= 0
            or resources.memory_mb <= 0
            or resources.pids_limit <= 0
        ):
        ...
        if not allowed or default not in allowed or len(set(allowed)) != len(allowed):
            raise AgentDefinitionError(
                "default model must occur once in a non-empty allowed list"
            )
        self._validate_opencode(
            opencode, allowed, default, path, gateway_url=self.gateway_url
        )
        return AgentDefinition(
            configured_id,
            parser.get("agent", "display_name"),
            image,
            idle,
            resources,
            default,
    ...
    def _validate_opencode(
        ...
        *,
        ...
        if not isinstance(plugins, list) or any(
            not isinstance(item, str) or not item.startswith("file://")
            for item in plugins
        ):
        ...
        try:
            ...
            raise AgentDefinitionError(
                "cloud-model-gateway provider is incomplete"
            ) from exc
        if (
            ...
            raise AgentDefinitionError(
                "provider models or local gateway URL do not match agent.cfg"
            )
        if config.get("model") != f"cloud-model-gateway/{default}":
            raise AgentDefinitionError(
                "OpenCode default model does not match agent.cfg"
            )
    ...
    def _validate_local_reference(
        ...
        try:
            matches = (
                list(agent_path.glob(relative))
                if allow_glob
                else [agent_path / relative]
            )
        except (OSError, ValueError) as exc:
            raise AgentDefinitionError(
                f"invalid local reference {value!r}: {exc}"
            ) from exc
        if not matches:
        ...
        for match in matches:
            ...
            if not resolved.is_relative_to(agent_path) or not resolved.is_file():
                raise AgentDefinitionError(
                    f"local reference is outside Agent definition: {value}"
                )
