...
from shared_libs.config_models import LoadCapacityConfig
...
def cpu_ticks():
    ...
    values = [
        int(v) for v in Path("/proc/stat").read_text().splitlines()[0].split()[1:9]
    ]
...
def host_sample():
    ...
    return {
        "memory_mb": values["MemTotal"] / 1024,
        "memory_available_mb": values["MemAvailable"] / 1024,
        "cpu_usage_percent": max(
            0, min(100, 100 * (1 - (after[1] - before[1]) / elapsed))
        ),
    }
...
def calculate(info, host, containers, policy):
    ...
    if (
        total_cpu <= 0
        or total_memory <= 0
        or abs(total_memory - host["memory_mb"]) > total_memory * 0.05
    ):
        raise ValueError("Docker daemon and /proc host do not agree")
    cpu, memory, growth = 0.0, 0.0, 0.0
    unknown = []
    ...
    for item in containers:
        ...
        if (
            not limit_cpu
            and config.get("CpuQuota", 0) > 0
            and config.get("CpuPeriod", 0) > 0
        ):
    ...
    reserved_memory = max(
        policy.reserved_memory_mb, total_memory * policy.reserve_fraction
    )
    remaining_cpu = total_cpu - reserved_cpu - cpu
    remaining_memory = min(
        total_memory - reserved_memory - memory,
    ...
    return {
        ...
        "allocated_cpu": round(cpu, 3),
        "allocated_memory_mb": round(memory, 1),
        "reserved_cpu": round(reserved_cpu, 3),
        ...
        "unbounded_sandboxes": len(unknown),
...
class LoadCapacity:
    def __init__(self, backend):
        ...
        self.policy = getattr(
            getattr(backend, "config", None), "load_capacity", LoadCapacityConfig()
        )
        self._task = None
        self._cached = None
    ...
    def _sample(self):
        ...
        def inspect(container):
            ...
            cache = memory_stats.get(
                "total_cache", memory_stats.get("cache", memory_stats.get("file", 0))
            )
            ...
            return {
                ...
                "memory_usage_mb": working / 1024**2,
            }
    ...
    async def snapshot(self, fresh=False):
        ...
        try:
            ...
            return {
                ...
                "reasons": ["无法读取服务器资源状态，请检查 Docker 和宿主机监测"],
