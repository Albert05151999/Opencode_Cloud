...
async def directory_collection(
    client, registry, endpoint, path="/session/status", timeout=5
):
    """Native status/permission collections are scoped to an OpenCode directory."""
    routes = await asyncio.to_thread(
        registry.list_sessions_for_sandbox, endpoint.sandbox_id
    )
    directories = [
        "/workspace",
        *[f"/workspace/sessions/{r.session_id}" for r in routes],
    ]
    ...
    async def ensure(self, endpoint, route):
        ...
        async with lock:
            await asyncio.to_thread(
                self.workspaces.user_scope, route.agent_id, route.username, sid
            )
            for name in ("inputs", "outputs"):
                await asyncio.to_thread(
                    self.workspaces.resolve_user_path,
                    route.agent_id,
                    route.username,
                    name + "/.placeholder",
                    sid,
            ...
            try:
                ...
                if session.get("directory") != directory:
                    moved = await self.client.post(
                        endpoint.base_url + "/experimental/control-plane/move-session",
                        json={
                            "sessionID": sid,
                    ...
                    if moved.status_code >= 400:
                        ...
                        raise HTTPException(
                            409,
                            "Session directory binding failed; wait for active tasks to finish and retry",
                        )
                    response = await self.client.get(
                        endpoint.base_url + f"/session/{sid}"
                    )
                    response.raise_for_status()
                    session = response.json()
                    if session.get("directory") != directory:
                        raise HTTPException(
                            502, "Native session directory binding was not confirmed"
                        )
                return session
            except (httpx.HTTPError, ValueError) as exc:
                raise HTTPException(
                    502, "Unable to verify native session directory"
                ) from exc
