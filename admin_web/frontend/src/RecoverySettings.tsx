...
export function RecoverySettings() {
  ...
  useEffect(() => {
    ...
    remote("/cloud/admin/recovery-policy")
      .then((value) => {
        if (active && Array.isArray(value.backoff_seconds)) {
          setPolicy(value);
          setBackoff(value.backoff_seconds.join(","));
        }
      })
      .catch((e) => {
    ...
    remote("/cloud/agents")
      .then((v) => {
        if (active && Array.isArray(v)) setAgents(v.map((a: Dict) => a.id));
      })
      .catch(() => {});
    return () => {
      active = false;
  ...
  return (
    <details className="panel">
      <summary>异常沙箱自动恢复设置</summary>
      <p className="muted">
        只恢复能够确认空闲的异常沙箱。任务忙碌或未知、人工停止、Agent
        归档及依赖服务异常时跳过。与配置发布共享操作队列，不会自动重发聊天。
      </p>
      ...
      {policy && (
        ...
        >
          ...
          {[
            ["failure_threshold", "异常观察次数", 1, 20],
            ["max_attempts", "时间窗口内最多恢复次数", 1, 10],
            ["window_seconds", "重试窗口（秒）", 60, 86400],
            ["stable_seconds", "健康稳定后重置计数（秒）", 30, 3600],
          ...
          {agents.map((id) => (
            <label className="checkbox" key={id}>
              <input
                type="checkbox"
                checked={policy.agent_enabled?.[id] !== false}
                onChange={(e) =>
                  setPolicy({
                    ...policy,
                    agent_enabled: {
