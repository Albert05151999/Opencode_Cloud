...
export function ProviderFields({
  ...
  useEffect(() => {
    let active = true;
    remote("/cloud/admin/provider-templates")
      .then((v) => {
        if (active && Array.isArray(v)) setTemplates(v);
      })
      .catch((e) => {
  ...
  return (
    <section className="panel">
      <label>
        ...
        >
          <option value="">自定义 / 保持现有配置</option>
          {templates.map((t) => (
      ...
      {template && (
        <p className="muted">
          ...
          {template.example_model &&
            `示例模型：${template.example_model}（需账户支持）`}{" "}
      ...
      <p className="muted">
        平台 ID 是 Agent 选择的名称；上游模型 ID 是厂商实际模型名称。保存 →
        发布模型网关 → 分配给 Agent → 发布 Agent。
      </p>
      <button
        type="button"
        className="secondary"
        disabled={busy || value.legacy}
        onClick={async () => {
          setBusy(true);
          setError("");
          setResult(null);
      ...
      {result && (
        <p role="status">
          {result.ok ? "当前表单测试通过" : `测试失败：${result.error}`} ·
          全局网关未修改
        </p>
      )}
    </section>
