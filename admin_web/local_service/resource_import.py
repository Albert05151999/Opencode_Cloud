...
from admin_web.local_service.formats import fail
from admin_web.local_service.formats import FORMAT, LIMIT, parse_native, skill_item
from admin_web.local_service.importer import jsonc
...
def collect(importer, file, include_external_skills=False):
    ...
    sources = [
        s
        for s in importer.discover(file=file)["sources"]
        if importer.sources[s["id"]] == requested
    ]
    if len(sources) != 1:
    ...
    config = importer.resolve_value(
        {k: v for k, v in raw.items() if k in {"mcp", "model", "small_model"}},
        source.parent,
    )
    ...
    if raw.get("provider"):
        ...
        if preview["errors"]:
            fail(
                "Model import errors: "
                + "; ".join(str(e.get("error")) for e in preview["errors"])
            )
        models = importer.take(
            preview["preview_id"], [m["id"] for m in preview["models"]]
        )
        items.extend(
            {"id": m["id"], "kind": "model", "name": m.get("name", m["id"]), "data": m}
            for m in models
        )
    ...
    for declared in paths:
        ...
        if (
            not resolved.is_relative_to(source.parent.resolve())
            and not include_external_skills
        ):
            fail(
                "Skill path is outside the configuration directory; explicitly enable external Skill paths"
            )
        roots = (
            [path]
            if (path / "SKILL.md").is_file()
            else [p.parent for p in path.glob("*/SKILL.md")]
        )
        for root in roots:
    ...
    result = json.dumps(
        {"format": FORMAT, "format_version": 1, "items": items, "warnings": warnings}
    ).encode()
    if len(result) > LIMIT:
