...
from operations.src.admin_dto import DTO, ResourceID
...
class LoadAgent(DTO):
    ...
    cpu_limit: float = Field(1, ge=0.25, le=64, allow_inf_nan=False)
...
def summarize(users):
    ...
    ends = [
        u["finished_at"]
        for u in users
        if u.get("sent_at") is not None and u.get("finished_at")
    ]
    ...
    return {
        "users": len(users),
        "prepared": sum(bool(u.get("session_id")) for u in users),
        "submitted": len(starts),
        "succeeded": len(successful),
        "failed": sum(
            u["phase"] in {"prepare_failed", "failed", "timed_out", "interrupted"}
            for u in users
        ),
        ...
        "launch_spread_ms": round((max(starts) - min(starts)) * 1000, 2)
        if starts
        else None,
...
class LoadTestStore:
    def __init__(self, store):
        ...
        with store.connect() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS load_tests (id TEXT PRIMARY KEY, request_id TEXT UNIQUE NOT NULL, fingerprint TEXT NOT NULL, status TEXT NOT NULL, created_at REAL NOT NULL, document TEXT NOT NULL)"
            )
            db.execute(
                "CREATE TABLE IF NOT EXISTS load_test_users (username TEXT PRIMARY KEY, agent_id TEXT NOT NULL, run_id TEXT NOT NULL REFERENCES load_tests(id), state TEXT NOT NULL, document TEXT NOT NULL)"
            )
            db.execute(
                "CREATE INDEX IF NOT EXISTS load_test_users_run ON load_test_users(run_id)"
            )
    ...
    def validate_agents(self, request):
        ...
        for selected in request.agents:
            ...
            if (
                ...
                or agent.get("lifecycle", "active") != "active"
            ):
                fail(
                    "Select an existing, published and enabled Agent: "
                    + selected.agent_id,
                    409,
                )
            snapshots[selected.agent_id] = {
    ...
    def create(self, request):
        ...
        with self.store.lock, self.store.connect() as db:
            ...
            previous = db.execute(
                "SELECT id,fingerprint FROM load_tests WHERE request_id=?",
                (request.request_id,),
            ).fetchone()
            if previous:
                if previous[1] != fingerprint:
                    fail(
                        "Request ID was already used with different test parameters",
                        409,
                    )
                return previous[0], False
            placeholders = ",".join("?" for _ in ACTIVE)
            if db.execute(
                f"SELECT 1 FROM load_tests WHERE status IN ({placeholders})",
                tuple(ACTIVE),
            ).fetchone():
                fail("A load test is already active", 409)
            targets = {a.agent_id for a in request.agents}
            for row in db.execute("SELECT document FROM jobs"):
                ...
                if job["status"] in {
                    ...
                    or job.get("scope", job["target"]) in targets
                ):
                    fail("An operation holds a selected Agent", 409)
            ...
            document = {
                ...
                "scope": "server_initiated_controller_sandbox_model",
            }
            db.execute(
                "INSERT INTO load_tests VALUES (?,?,?,?,?,?)",
                (
                    rid,
                    request.request_id,
                    fingerprint,
            ...
            for agent in request.agents:
                for _ in range(agent.users):
                    ...
                    db.execute(
                        "INSERT INTO load_test_users VALUES (?,?,?,?,?)",
                        (username, agent.agent_id, rid, "retained", json.dumps(user)),
                    )
            return rid, True
    ...
    def get(self, rid):
        with self.store.connect() as db:
            row = db.execute(
                "SELECT status,document FROM load_tests WHERE id=?", (rid,)
            ).fetchone()
            if not row:
                fail("Load test not found", 404)
            users = [
                dict(json.loads(r[1]), storage_state=r[0])
                for r in db.execute(
                    "SELECT state,document FROM load_test_users WHERE run_id=? ORDER BY username",
                    (rid,),
                )
            ]
        ...
        report["by_agent"] = {
            aid: summarize([u for u in users if u["agent_id"] == aid])
            for aid in report["snapshots"]
        }
        return report
    ...
    def list(self, offset=0, limit=25):
        with self.store.connect() as db:
            ...
            rows = db.execute(
                "SELECT id,status,document FROM load_tests ORDER BY created_at DESC LIMIT ? OFFSET ?",
                (limit, offset),
            ).fetchall()
            placeholders = ",".join("?" for _ in ACTIVE)
            active = db.execute(
                f"SELECT id FROM load_tests WHERE status IN ({placeholders}) LIMIT 1",
                tuple(ACTIVE),
            ).fetchone()
        return {
            "total": total,
    ...
    def active_user(self, aid, username):
        ...
        with self.store.connect() as db:
            row = db.execute(
                "SELECT r.status,r.document FROM load_test_users u JOIN load_tests r ON r.id=u.run_id WHERE u.username=? AND u.agent_id=?",
                (username, aid),
            ).fetchone()
        return bool(
            row
    ...
    def update(self, rid, **values):
        with self.store.connect() as db:
            ...
            db.execute(
                "UPDATE load_tests SET status=?,document=? WHERE id=?",
                (status, json.dumps(document), rid),
            )
    ...
    def user(self, agent_id, username):
        ...
        with self.store.connect() as db:
            row = db.execute(
                "SELECT state,document FROM load_test_users WHERE username=? AND agent_id=?",
                (username, agent_id),
            ).fetchone()
    ...
    def update_user(self, username, **values):
        with self.store.connect() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute(
                "SELECT state,document FROM load_test_users WHERE username=?",
                (username,),
            ).fetchone()
            if not row:
            ...
            db.execute(
                "UPDATE load_test_users SET state=?,document=? WHERE username=?",
        ...
        for rid in ids:
            ...
            if report["status"] in ACTIVE:
                self.update(
                    ...
                    error="Controller restarted; requests were not replayed",
                )
            ...
            if report["cleanup_status"] == "cleaning":
                self.update(
                    ...
                    cleanup_error="Controller restarted; retry cleanup",
                )
