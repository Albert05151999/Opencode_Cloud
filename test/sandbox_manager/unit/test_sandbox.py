    ...
    def list(self, *, all=False, filters=None):
        ...
        if label:
            ...
            return [
                ...
                if c.attrs["Config"]["Labels"].get(key) == value
            ]
...
def test_release_keeps_lease_during_write_and_drops_it_on_failure(
...
def test_cold_probe_uses_short_slice_without_changing_warm_probe(backend_factory):
    ...
    backend.config = replace(
        backend.config,
        sandbox=replace(backend.config.sandbox, start_timeout_seconds=10),
    )
...
def test_different_users_and_agents_have_isolated_mounts(backend_factory: Any) -> None:
    ...
    workspace_sources = [
        next(
            source
            for source, target in volume.items()
            if target["bind"] == "/workspace"
        )
...
def test_stopped_owned_container_is_started_without_create(
    backend_factory: Any,
) -> None:
    backend, registry, client, _ = backend_factory()
    ...
    container = FakeContainer(
        "existing",
        f"cloud-agent-{key}",
        owned_labels(backend, "agent-code", "alice"),
        4096,
        42000,
        status="exited",
    )
...
def test_missing_recorded_container_is_rebuilt(backend_factory: Any) -> None:
    ...
    assert len(client.containers.create_calls) == 1
    assert (
        registry.get_sandbox("agent-code", "alice").container_id
        == endpoint.container_id
    )
...
def test_label_mismatch_is_rejected(backend_factory: Any) -> None:
    ...
    client.containers.add(
        FakeContainer(
            "wrong-owner", f"cloud-agent-{key}", labels, 4096, 42000, status="running"
        )
    )
...
def test_health_version_mismatch_fails_and_removes_container(
    backend_factory: Any,
) -> None:
    ...
    created = next(
        iter({c.id: c for c in client.containers.by_identifier.values()}.values())
    )
    assert created.remove_calls == [True]
    record = registry.get_sandbox("agent-code", "alice")
    assert record is not None
    assert (record.status, record.container_id, record.host_port) == (
        "failed",
        None,
        None,
    )
...
def test_create_uses_hardened_resources_mounts_and_networking(
    backend_factory: Any,
) -> None:
    backend, _, client, agents = backend_factory()
    ...
    assert kwargs["environment"] == {
        "SANDBOX_ID": "sbx_" + sandbox_key("agent-code", "alice"),
    ...
    assert sorted(entry["mode"] for entry in volumes.values()) == [
        "ro",
        "rw",
        "rw",
        "rw",
...
def test_recovery_repairs_stale_docker_route_preserves_sessions_and_marks_missing(
    backend_factory,
):
    backend, registry, client, _ = backend_factory()
    ...
    async def scenario():
        ...
        registry.record_session(
            "ses_recovery", first.sandbox_id, "agent-code", "alice", "sessions/custom"
        )
        ...
        registry.upsert_sandbox(
            ...
            host_port=1,
            status="unhealthy",
            image_version=original.image_version,
            last_active_at=original.last_active_at,
        )
        client.containers.by_identifier = {
            k: v
            for k, v in client.containers.by_identifier.items()
            if v.id != second.container_id
        }
        assert await backend.reconcile() == {"observed": 1, "missing": 1}
        recovered = registry.get_sandbox("agent-code", "alice")
        assert (
            recovered.container_id == first.container_id
            and recovered.host_port == first.host_port
        )
        assert (
            recovered.status == "ready"
            and recovered.last_active_at == original.last_active_at
        )
        assert (
            registry.get_session_route("ses_recovery").workspace_relpath
            == "sessions/custom"
        )
...
def test_runtime_upgrade_preserves_active_agent_image_and_recovery_paths(
    backend_factory,
):
    backend, registry, client, _ = backend_factory()
    ...
    async def scenario():
        first = await backend.acquire("agent-code", "alice")
        await backend.release(first)
        old_image = registry.get_sandbox("agent-code", "alice").image_version
        backend.management = SimpleNamespace(
            load_tests=SimpleNamespace(resources_for=lambda *args: None)
        )
        backend.config = replace(
            backend.config,
            sandbox=replace(backend.config.sandbox, image="runtime-next"),
        )
        # Updating the runtime default does not rewrite an already published Agent.
        assert await backend.reconcile() == {"observed": 1, "missing": 0}
        assert registry.get_sandbox("agent-code", "alice").image_version == old_image
        second = await backend._acquire_locked("agent-code", "bob")
        assert client.containers.create_calls[-1]["image"] == old_image
        assert registry.get_sandbox("agent-code", "bob").image_version == old_image
        with registry.connect() as db:
            assert Path(
                db.execute(
                    "SELECT config_path FROM agents WHERE agent_id='agent-code'"
                ).fetchone()[0]
            ).is_dir()
...
def test_managed_acquire_uses_one_authoritative_definition(backend_factory):
    backend, registry, client, _ = backend_factory()
    backend.management = SimpleNamespace(load_tests=SimpleNamespace(resources_for=lambda *args: None))
    original = backend.agent_catalog.load
    calls = []
    def load(agent_id):
        calls.append(agent_id)
        return original(agent_id)
    backend.agent_catalog.load = load
    async def scenario():
        first = await backend.acquire("agent-code", "alice")
        ...
        calls.clear()
        second = await backend.acquire("agent-code", "alice")
        assert second.reused and calls == ["agent-code"]
        await backend.release(second)
    run(scenario())


def test_acquire_stage_timings_preserve_cold_and_hot_checks(backend_factory, monkeypatch):
    ...
    def stage(name):
        stages.append(name)
        yield
    monkeypatch.setattr('sandbox_manager.backend.timed_stage',stage)
    backend, _, _, _ = backend_factory()
    async def scenario():
        first=await backend.acquire('agent-code','alice')
        await backend.release(first)
        assert {'lock_wait','catalog','workspace','docker_lookup','docker_create','docker_start','readiness'} <= set(stages)
        stages.clear()
        second=await backend.acquire('agent-code','alice')
        await backend.release(second)
        assert {'lock_wait','catalog','workspace','docker_lookup','readiness'} <= set(stages)
        assert 'docker_create' not in stages
    run(scenario())


def test_remote_admission_once_under_lock_and_rechecks_queued_block(backend_factory):
    from sandbox_manager.catalog_client import Admission
    from fastapi import HTTPException
    backend, _, _, _ = backend_factory()
    admission=object.__new__(Admission)
    admission.blocked=set();admission.operations=admission;admission.load_tests=admission
    calls=[]
    admission.resources_for=lambda *args: calls.append(args)
    backend.management=admission
    async def scenario():
        # Cold creation separately requests resource sizing; warm admission does not.
        first=await backend.acquire('agent-code','alice')
        await backend.release(first);calls.clear()
        second=await backend.acquire('agent-code','alice')
        await backend.release(second)
        assert calls==[('agent-code','alice')]
        lock=asyncio.Lock();backend._locks[('agent-code','alice')]=lock
        await lock.acquire()
        task=asyncio.create_task(backend.acquire('agent-code','alice'))
        await asyncio.sleep(.01)
        admission.blocked.add('agent-code');lock.release()
...
def test_workspace_and_inspection_overlap_and_drain_failure(backend_factory):
    import threading
    backend, _, client, _ = backend_factory()
    workspace_started=threading.Event();inspection_started=threading.Event();finished=threading.Event()
    original=backend.workspaces.ensure_user_layout
    def workspace(*args):
        workspace_started.set()
        assert inspection_started.wait(2), 'Independent inspection should overlap allocation'
        layout=original(*args)
        finished.set()
        return layout
    async def inspection(*args):
        assert await asyncio.to_thread(workspace_started.wait,2)
        inspection_started.set()
        raise SandboxError('inspection failed')
    backend.workspaces.ensure_user_layout=workspace
    backend._find_existing=inspection
    with pytest.raises(SandboxError,match='inspection failed'):
        run(backend.acquire('agent-code','alice'))
    assert finished.is_set(), 'Allocation must finish before failed acquire releases its lock'
    assert not client.containers.create_calls


def test_repeated_cancellation_drains_allocation_before_unlock(backend_factory):
    import threading
    backend, _, _, _ = backend_factory()
    ...
    async def scenario():
        ...
        lock=backend._locks[('agent-code','alice')]
        try:
