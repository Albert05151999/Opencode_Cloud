...
import {
  splitThinking,
  displayParts,
} from "../../../../admin_web/frontend/src/thinking.ts";

test("separates thinking and answer, preserving order and multiple blocks", () => {
  assert.deepEqual(
    splitThinking("before<think>reason</think>answer<think>more</think>end"),
    [
...
test("every stream prefix keeps reasoning out of the answer and hides partial tags", () => {
  ...
  for (let i = 1; i <= text.length; i++) {
    ...
    assert.ok(
      parts.every((p) => !p.text.includes("<") && !p.text.includes(">")),
    );
  }
});
test("unclosed historical thinking stays separate and final literal prefixes are preserved", () => {
  assert.deepEqual(splitThinking("<think>unfinished"), [
    { type: "reasoning", text: "unfinished" },
  ]);
  assert.deepEqual(splitThinking("answer <thi"), [
    { type: "text", text: "answer <thi" },
  ]);
});
...
test("adjacent text parts support split tags, native reasoning and tools retain order", () => {
  const parts = displayParts(
    [
      { id: "a", type: "text", text: "<thi" },
      { id: "b", type: "text", text: "nk>reason</think>answer" },
      { id: "c", type: "tool" },
      { id: "d", type: "reasoning", text: "native" },
    ],
    true,
  );
  assert.deepEqual(
    parts.map((p) => [p.type, p.text]),
    [
      ["reasoning", "reason"],
      ["text", "answer"],
      ["tool", undefined],
