...
import api_gateway.api_docs as api_docs
...
def test_static_doc_preserves_native_schema_and_adds_routing_contract() -> None:
    ...
    native_responses = copy.deepcopy(
        source["paths"]["/session/{sessionID}"]["get"]["responses"]
    )
    ...
    assert (
        document["paths"]["/session/{sessionID}"]["get"]["responses"]
        == native_responses
    )
...
def test_router_has_no_backend_dependency_and_missing_spec_fails_locally(
