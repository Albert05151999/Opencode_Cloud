...
from shared_libs.config_models import ConfigurationError, load_config, parse_csv
...
def test_sqlite_sync_policy_is_explicit_and_rejects_off(tmp_path):
    assert (
        load_config(ROOT / "config.cfg", environ={}).platform.sqlite_synchronous
        == "NORMAL"
    )
    assert (
        load_config(
            ROOT / "config.cfg",
            environ={"CLOUD_AGENT__PLATFORM__SQLITE_SYNCHRONOUS": "FULL"},
        ).platform.sqlite_synchronous
        == "FULL"
    )
    with pytest.raises(ConfigurationError, match="NORMAL or FULL"):
        load_config(
            ROOT / "config.cfg",
            environ={"CLOUD_AGENT__PLATFORM__SQLITE_SYNCHRONOUS": "OFF"},
        )
...
def test_rejects_relative_storage_path(tmp_path: Path) -> None:
    path = config_copy(
        tmp_path,
        ("workspace_root = /srv/cloud-agent/workspaces", "workspace_root = relative"),
    )
...
def test_rejects_disabled_startup_safety_flag(tmp_path: Path) -> None:
    path = config_copy(
        tmp_path,
        ("startup_disable_autoupdate = true", "startup_disable_autoupdate = false"),
    )
