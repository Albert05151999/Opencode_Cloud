...
from catalog_service.src.transfers import Transfers, FORMAT
...
def choices(preview, suffix=""):
    return [
        {"key": e["key"], "target_id": e["id"] + suffix, "replace": False}
...
def test_native_import_is_staged_and_never_assigns_agents(service):
    ...
    assert "test-secret-key" not in json.dumps(
        preview
    ) and "private-header" not in json.dumps(preview)
...
def test_skill_roundtrip_preserves_content(service):
    ...
    _, data = service.store.read()
    assert (
        data["resources"]["example-skill-copy"]["draft"]["files"]
        == data["resources"]["example-skill"]["draft"]["files"]
    )
...
def test_future_format_and_empty_selection_rejected(service):
    with pytest.raises(HTTPException):
        service.preview(
            "resources.json",
            json.dumps({"format": FORMAT, "format_version": 99}).encode(),
        )
