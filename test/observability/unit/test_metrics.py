...
from test.support import create_router_app as create_app
from shared_libs.metrics import MetricsMiddleware, PlatformMetrics, route_label
...
def test_requests_are_counted_and_metric_registries_are_isolated():
    ...
    assert (
        'cloud_http_requests_total{method="GET",route="/cloud/health",status="200"} 1.0'
        in body
    )
    assert (
        'cloud_http_errors_total{method="GET",route="other",status="404"} 1.0' in body
    )
    assert "private-user" not in body
    assert (
        second.registry.get_sample_value(
            "cloud_http_requests_total",
            {"method": "GET", "route": "/cloud/health", "status": "200"},
        )
        is None
    )
    ...
    async def exercise():
        ...
        task = asyncio.create_task(
            MetricsMiddleware(app, metrics)(
                {"type": "http", "method": "GET", "path": "/event"}, receive, send
            )
        )
        await opened.wait()
        ...
        assert (
            metrics.registry.get_sample_value(
                "cloud_sse_connection_duration_seconds_count"
            )
            == 1
        )
