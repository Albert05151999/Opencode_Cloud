...
Write the human-readable content of project planning files in Chinese by default, including goals, phase names, checklist items, findings, decisions, progress notes, errors, and test results. Preserve the English structural markers required by the scripts: `## Goal`, `## Next Step`, `## Current Phase`, `## Phases`, `### Phase`, `**Status:**`, and the status values `pending`, `in_progress`, and `complete`. Existing planning files may keep their original language unless the user asks to translate them.
