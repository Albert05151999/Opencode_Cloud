...
from local_web.runtime import LocalServer, until_stopped
...
class LocalShutdownTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_shutdown_ends_live_proxy_without_browser_disconnect(self):
        ...
        async def start(app, cls):
            ...
            sockets.append(sock)
            server = cls(uvicorn.Config(app, log_level='error', timeout_graceful_shutdown=2))
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as root:
                app = create_local_app(root, SimpleNamespace(url=upstream_url, token='test'))
