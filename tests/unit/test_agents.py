...
REPOSITORY_AGENTS = Path(__file__).resolve().parents[2] / "agents"
...
def test_rejects_provider_model_or_gateway_drift(
...
def test_rejects_nonlocal_plugins(tmp_path: Path, plugin: str) -> None:
    opencode = valid_opencode()
    opencode["plugin"] = [plugin]
    write_agent(tmp_path, opencode=opencode)
    with pytest.raises(AgentDefinitionError, match="only local"):
        AgentCatalog(tmp_path).load("agent-code")


@pytest.mark.parametrize(
    ("field", "value"),
    [("share", "manual"), ("share", True), ("autoupdate", True)],
)
def test_rejects_sharing_or_automatic_updates(
    tmp_path: Path, field: str, value: object
) -> None:
    opencode = valid_opencode()
    opencode[field] = value
    write_agent(tmp_path, opencode=opencode)
    with pytest.raises(AgentDefinitionError, match="sharing must be disabled"):
        AgentCatalog(tmp_path).load("agent-code")
...
def test_rejects_unapproved_provider_package(tmp_path: Path) -> None:
    opencode = valid_opencode()
    opencode["provider"]["cloud-model-gateway"]["npm"] = "remote-package"
    write_agent(tmp_path, opencode=opencode)
    with pytest.raises(AgentDefinitionError, match="provider models"):
        AgentCatalog(tmp_path).load("agent-code")
