...
from test_transfers import service, choices, SKILL
...
def test_migration_keeps_fixed_versions_and_templates_separate(service):
    ...
    store.publish_resource('versioned', copy.deepcopy(store.read()[1]['resources']['versioned']['draft']))
    ...
    template = next(t for t in AgentTemplates(store).list() if t['source_id']=='source-draft')
    ...
    assert restored['active'] is None and restored['draft']['bindings'][0]['version'] == 1
...
def test_resource_file_checksum_blocks_tampered_import(service):
    ...
    assert any(i.get('error')=='Resource file checksum mismatch' for i in preview['items'])
...
def test_legacy_models_export_as_managed_models_with_both_routes(service):
    ...
    for mid in service.store.read()[1]['models']:
        ...
        env.update({prefix+'_MODEL':'openai/upstream',prefix+'_API_KEY':'legacy-key',
            prefix+'_1_API_BASE':'https://one.example/v1',prefix+'_2_API_BASE':'https://two.example/v1'})
    service.runtime=SimpleNamespace(gateway_container=lambda:SimpleNamespace(attrs={'Config':{'Env':[k+'='+v for k,v in env.items()]}}))
    ...
    assert [i['litellm_params']['api_base'] for i in compiled['model_list']]==['https://one.example/v1','https://two.example/v1']
