...
文档定位（2026-09-12）：本文件为基础阶段历史路线与验证要求，不再是全项目唯一进度来源。后续实现见 04～06 阶段文档，当前总览见 [项目进度](项目进度.md)。下方运行规则的复选框是初始执行约定，不代表当前仍有同名功能未完成；逐项历史结果保存在 `docs/progress.json` 的 `tasks`。
...
- [ ] 基础阶段按本文有序执行；跨阶段状态以根目录《项目进度》及对应阶段记录为准。
