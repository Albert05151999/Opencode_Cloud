...
from app.management import fail, identifier, redact
...
def remap_template(template, mappings):
    ...
    config['allowed_model_ids'] = [mappings.get(('model', mid), mid) for mid in config['allowed_model_ids']]
...
class AgentTemplates:
    ...
    def restore(self, tid, aid, models=None, resources=None):
        ...
        with self.store.edit() as data:
            ...
            model_map = models or {}
            cfg['allowed_model_ids'] = [model_map.get(mid, mid) for mid in cfg['allowed_model_ids']]
            cfg['default_model_id'] = model_map.get(cfg['default_model_id'], cfg['default_model_id'])
            if cfg.get('small_model_id'):
                cfg['small_model_id'] = model_map.get(cfg['small_model_id'], cfg['small_model_id'])
            ...
            data['agents'][aid] = {'id': aid, 'draft': cfg, 'active': None, 'versions': []}
