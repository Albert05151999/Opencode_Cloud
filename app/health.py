...
class HealthMonitor:
    def __init__(self, backend: LocalDockerBackend, client: httpx.AsyncClient, metrics: PlatformMetrics) -> None:
    ...
    async def model_health(self) -> bool:
        try:
            response = await self.client.get(self.backend.config.model_gateway.health_url, timeout=3)
    ...
    async def readiness(self) -> dict[str, bool]:
        def local_checks():
            ...
            checks = {
                ...
                "image": lambda: self.backend.client.images.get(self.backend.config.sandbox.image),
            ...
            for name, check in checks.items():
                try:
                    ...
                    result[name] = False
            return result
        local, model = await asyncio.gather(asyncio.to_thread(local_checks), self.model_health())
    ...
    async def _check_sandbox(self, snapshot, now: datetime | None) -> None:
        ...
        async with lock:
            ...
            if record is None or record.status not in {"ready", "unhealthy", "stopped"}:
                return
            elapsed = ((now or datetime.now(timezone.utc)) - datetime.fromisoformat(record.last_active_at)).total_seconds()
            timeout = self.backend.agent_catalog.load(record.agent_id).idle_timeout_seconds
            ...
            try:
                ...
                payload = await self.backend._health_probe(
                    endpoint.base_url + self.backend.config.opencode.health_path, 2,
                )
                if payload.get("healthy") is not True or payload.get("version") != self.backend.config.opencode.expected_version:
                    raise RuntimeError("OpenCode health/version mismatch")
            except Exception:
                self.metrics.sandbox_health_failures.inc()
                self.failures[record.sandbox_id] = self.failures.get(record.sandbox_id, 0) + 1
                if self.failures[record.sandbox_id] >= self.backend.config.sandbox.health_failure_threshold:
                    await asyncio.to_thread(self.backend.registry.set_health_status, record.sandbox_id, "unhealthy")
                return
            self.failures.pop(record.sandbox_id, None)
            if record.status == "unhealthy":
                await asyncio.to_thread(self.backend.registry.set_health_status, record.sandbox_id, "ready")
            if self.backend.in_use.get(record.sandbox_id, 0):
                await asyncio.to_thread(self.backend.registry.touch_sandbox, record.sandbox_id)
                return
            elapsed = ((now or datetime.now(timezone.utc)) - datetime.fromisoformat(record.last_active_at)).total_seconds()
            timeout = self.backend.agent_catalog.load(record.agent_id).idle_timeout_seconds
            if elapsed < timeout:
            ...
            try:
                ...
                sessions = await directory_collection(self.client, self.backend.registry, endpoint, timeout=2)
                if not isinstance(sessions, dict) or any(value.get("type") != "idle" for value in sessions.values()):
                    await asyncio.to_thread(self.backend.registry.touch_sandbox, record.sandbox_id)
                    return
    ...
    async def _remove_idle_container(self, record, container):
        ...
        await asyncio.to_thread(
            ...
            username=record.username, container_id=None, host_port=None,
            status="stopped", image_version=record.image_version,
...
def create_health_router(monitor: HealthMonitor) -> APIRouter:
    ...
    async def readiness():
        ...
        return JSONResponse({"ok": ready, "checks": checks}, status_code=200 if ready else 503)
    return router
