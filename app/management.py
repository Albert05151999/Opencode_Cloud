...
MODEL_PARAMETERS = {'temperature', 'top_p', 'max_tokens', 'max_completion_tokens',
...
def redact(value):
    if isinstance(value, dict):
        return {k: ({n: MASK for n in v} if isinstance(v, dict) else MASK) if k in SECRET_FIELDS and v
...
def preserve_secrets(new, old):
    ...
    if isinstance(new, dict):
        return {k: preserve_secrets(v, old.get(k) if isinstance(old, dict) else None) for k, v in new.items()}
    return new
...
def relative_file(value):
    ...
    if path.is_absolute() or any(p in {'..', '.'} for p in value.split('/')) or not path.parts:
            ...
            with zipfile.ZipFile(io.BytesIO(content)) as archive:
                ...
                for entry in archive.infolist():
                    ...
                    if kind not in {0, stat.S_IFREG, stat.S_IFDIR} or entry.flag_bits & 1:
                        fail('Links, special files and encrypted ZIP entries are unsupported')
...
def validate_mcp(data):
    allowed = {'type', 'url', 'headers', 'timeout', 'enabled', 'command', 'cwd', 'environment', 'oauth'}
    if set(data) - allowed:
        fail('Unsupported MCP configuration field')
    if data.get('oauth', False) is not False:
        fail('MCP OAuth is not supported')
    if not isinstance(data.get('timeout', 10000), int) or not 1 <= data.get('timeout', 10000) <= 120000:
        fail('MCP timeout must be 1–120000 ms')
    for field in ('headers', 'environment'):
        if field in data and (not isinstance(data[field], dict) or any(
    ...
    if data.get('type') == 'remote':
        ...
        if executable in {'npx', 'npm', 'pnpm', 'yarn', 'uvx', 'pip', 'pip3', 'bash', 'sh', 'cmd', 'powershell'}:
            fail('Use an already installed MCP executable; installers and shell wrappers are unsupported')
        if data.get('cwd') and (not PurePosixPath(data['cwd']).is_relative_to('/workspace') or '..' in PurePosixPath(data['cwd']).parts):
...
def validate_model(model):
    ...
    if model.get('provider') not in {'openai', 'openai-compatible', 'anthropic', 'google', 'legacy'}:
    ...
    if any(not isinstance(v, (int, float, str)) or isinstance(v, bool) for v in parameters.values()):
        fail('Invalid model parameter value')
    if not isinstance(model.get('headers', {}), dict) or any(not isinstance(v, str) for v in model.get('headers', {}).values()):
...
class ManagementStore:
    ...
    def edit(self, expected=None):
        with self.lock, self.connect() as db:
            ...
            data = json.loads(document)
            yield data
            db.execute('UPDATE catalog SET revision=?, document=? WHERE id=1', (revision + 1, json.dumps(data, ensure_ascii=False)))
    ...
    def bootstrap(self):
        with self.edit() as data:
            ...
            for config in sorted(self.agents_root.glob('*/agent.cfg')):
                ...
                for mid in models:
                    data['models'].setdefault(mid, {'id': mid, 'name': mid, 'provider': 'legacy',
                        'upstream_model': mid, 'enabled': True, 'legacy': True})
                bindings = []
                for skill in sorted((config.parent / 'skills').glob('*/SKILL.md')):
                    ...
                    files = {p.relative_to(skill.parent).as_posix(): self.blob(p.read_bytes())
                             for p in skill.parent.rglob('*') if p.is_file() and not p.is_symlink()}
                    version = {'version': 1, 'created': time.time(), 'data': meta, 'files': files}
                    data['resources'][rid] = {'id': rid, 'kind': 'skill', 'name': meta['name'], 'owner': aid,
                        'archived': False, 'draft': {'data': meta, 'files': files}, 'versions': [version]}
                    bindings.append({'id': rid, 'version': 1})
                draft = {'name': cfg['agent']['display_name'], 'description': '', 'enabled': True,
                    'instructions': (config.parent / 'AGENTS.md').read_text(encoding='utf-8'),
                    'allowed_model_ids': models, 'default_model_id': cfg['models']['default'],
                    'small_model_id': None, 'bindings': bindings}
                data['agents'][aid] = {'id': aid, 'draft': draft, 'versions': [{'version': 1,
                    'created': time.time(), 'config': copy.deepcopy(draft), 'path': str(config.parent.resolve())}], 'active': 1}
    ...
    def resource_version(data, rid, version):
        ...
        result = next((v for v in resource['versions'] if v['version'] == version), None)
        if result is None:
    ...
    def agent_config(agent):
        version = next((v for v in agent['versions'] if v['version'] == agent['active']), None)
    ...
    def validate_agent(self, data, aid, cfg):
        ...
        for mid in allowed:
            if mid not in data['models'] or not data['models'][mid].get('enabled', True):
                fail('Agent references an unavailable model: ' + str(mid))
        if cfg.get('default_model_id') not in allowed or (cfg.get('small_model_id') and cfg['small_model_id'] not in allowed):
        ...
        for binding in cfg.get('bindings', []):
            resource, version = self.resource_version(data, binding['id'], binding['version'])
            ...
            names.add(key)
            ids.add(resource['id'])
        if not isinstance(cfg.get('instructions', ''), str) or len(cfg.get('instructions', '')) > 200000:
    ...
    def save_model(self, mid, value, expected=None):
        with self.edit(expected) as data:
            ...
            if value.get('provider') == 'legacy' and not data['models'].get(mid, {}).get('legacy'):
                fail('Legacy models can only be migrated from the existing deployment')
            if not value.get('enabled', True):
                refs = self.model_references(data, mid)
                if refs:
                    fail('Model is referenced by: ' + ', '.join(refs), 409)
            data['models'][mid] = value

    def model_references(self, data, mid):
        return [aid for aid, a in data['agents'].items() if mid in a['draft']['allowed_model_ids'] or
                mid in (self.agent_config(a) or {}).get('allowed_model_ids', [])]
    ...
    def save_agent(self, aid, value, expected=None, source=None):
        ...
        with self.edit(expected) as data:
            ...
            if old:
                ...
                data['agents'][aid] = {'id': aid, 'draft': value, 'versions': [], 'active': None}
    ...
    def save_resource(self, rid, value, expected=None):
        ...
        with self.edit(expected) as data:
            ...
            content = preserve_secrets(value.get('data', {}), old['draft']['data'] if old else {})
            ...
            if value['kind'] == 'mcp':
                ...
                if PurePosixPath(entry).suffix not in {'.js', '.mjs', '.ts'} or entry not in sources:
                ...
                files = {relative_file(p): self.blob(s.encode('utf-8')) for p, s in sources.items()}
                content = {'entry': entry, 'sources': sources}
            elif not old:
                fail('Create Skill resources using the upload endpoint')
            if old:
                old.update(name=name, archived=bool(value.get('archived', False)), draft={'data': content, 'files': files})
            else:
                data['resources'][rid] = {'id': rid, 'name': name, 'kind': value['kind'], 'owner': owner,
                    'archived': False, 'draft': {'data': content, 'files': files}, 'versions': []}
    ...
    def upload_skill(self, rid, filename, content, owner=None, expected=None):
        ...
        with self.edit(expected) as data:
            ...
            resource = old or {'id': rid, 'kind': 'skill', 'owner': owner, 'versions': [], 'archived': False}
    ...
    def new_job(self, kind, target, payload=None):
        ...
        with self.edit() as data:
            data['jobs'][jid] = {'id': jid, 'kind': kind, 'target': target, 'status': 'queued',
                'created': time.time(), 'payload': payload or {}, 'error': None}
        return jid
