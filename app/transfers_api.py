...
from app.admin_dto import DTO, ResourceID, VersionBinding
from app.transfers import Transfers, LIMIT
...
def create_transfers_router(store, runtime=None):
    ...
    from app.agent_templates import AgentTemplates
    templates = AgentTemplates(store)

    @router.get('/cloud/admin/exports/native/{aid}')
    def native_export(aid: str):
        from app.native_export import export_native
        return Response(export_native(runtime, aid), media_type='application/zip',
                        headers={'Content-Disposition':'attachment; filename="opencode-native.zip"'})
    ...
    def restore_template(tid: str, payload: TemplateRestore):
        return templates.restore(tid, payload.agent_id, payload.models, {key:value.model_dump() for key,value in payload.resources.items()})
    ...
    async def preview(file: UploadFile = File(), password: str | None = Form(None)):
        ...
        return await asyncio.to_thread(service.preview, file.filename or 'config.json', content, password)

    @router.post('/cloud/admin/exports/encrypted')
    def encrypted(payload: EncryptedExport):
        from app.encrypted_transfer import encrypt
        content = encrypt(service.export(payload.include_credentials), payload.password)
        return Response(content, media_type='application/json',
                        headers={'Content-Disposition': 'attachment; filename="cloud-resources.encrypted.json"'})
    ...
    def export():
        return Response(service.export(), media_type='application/json',
                        headers={'Content-Disposition': 'attachment; filename="cloud-resources.json"'})
