...
from app.registry import Registry, SessionRoute
from app.metrics import PlatformMetrics
from app.sandbox import LocalDockerBackend, SandboxEndpoint, SandboxError
from app.observability import bind_request, emit, safe_request_id
from app.session_binding import SessionBindings, flattened_events, directory_collection
...
_CLOUD_HEADERS = {"x-cloud-agent-id", "x-cloud-username", "x-cloud-request-id", "x-cloud-session-id"}
...
def create_gateway_router(
    ...
    bindings = SessionBindings(registry, sandboxes.workspaces, client) if hasattr(sandboxes, 'workspaces') else None
    ...
    async def proxy(request: Request, native_path: str) -> Response:
        ...
        if bindings and native_path in {'experimental/control-plane/move-session', 'experimental/workspace/warp'}:
        ...
        metadata = RouteMetadata(
            ...
            safe_request_id(metadata.request_id or request.scope.get("cloud_log", {}).get("request_id")),
        )
        ...
        if management:
            ...
            management_lease = management.authorize(agent_id, request.method, native_path, payload)
            if upstream_body and isinstance(payload, dict):
                upstream_body = json.dumps(payload).encode()
            if management_lease:
                management.lease(agent_id, True)
        bind_request(request.scope, request_id=metadata.request_id, component="gateway",
                     agent_id=agent_id, username=username, session_id=route.session_id if route else None)
        if upstream_body and request.headers.get("content-type", "").startswith("application/json"):
            try:
                native_payload = json.loads(upstream_body)
                if isinstance(native_payload, dict):
                    model = native_payload.get("model")
                    bind_request(request.scope, message_id=native_payload.get("messageID"),
                                 logical_model=model.get("modelID") if isinstance(model, dict) else None)
        ...
        context = bind_request(request.scope, sandbox_id=endpoint.sandbox_id, container_id=endpoint.container_id)
        ...
        headers = {
            ...
            if name.lower() not in _HOP_BY_HOP | _CLOUD_HEADERS | {"host", "content-length"} | ({'authorization'} if management else set())
        }
        headers["X-Cloud-Request-ID"] = metadata.request_id
        if bindings and request.method == 'POST' and native_path.rstrip('/') in {'session', 'api/session'}:
            url = str(httpx.URL(url).copy_remove_param('workspace').copy_set_param('directory', '/workspace'))
        ...
        if bindings and request.method == 'GET' and native_path == 'session':
            ...
            url = str(httpx.URL(endpoint.base_url + '/experimental/session').copy_merge_params(
                [(k, v) for k, v in request.query_params.multi_items() if k not in {'directory', 'workspace', 'path', 'scope'}]))
        if request.method == "GET" and (
            native_path in {"event", "global/event", "api/event"}
            or _SESSION_EVENT_PATH.fullmatch(native_path)
        ):
            return await _stream_sse(client, sandboxes, endpoint, url, headers, tracker, request, flatten=flatten)
        activity_persisted = False
        try:
            if bindings and not route and request.method == 'GET' and native_path in {'session/status', 'permission', 'question'}:
                try:
                    collection = await directory_collection(client, registry, endpoint, '/' + native_path)
                except (httpx.HTTPError, ValueError) as exc:
                    raise HTTPException(502, 'Could not read state across session directories') from exc
                return Response(content=json.dumps(collection), media_type='application/json')
            if bindings and route and request.method not in {'GET', 'HEAD', 'DELETE'} and not native_path.endswith(('/abort', '/interrupt')):
                await bindings.ensure(endpoint, route)
            if bindings and route:
                url = str(httpx.URL(url).copy_remove_param('workspace').copy_set_param('directory', f'/workspace/sessions/{route.session_id}'))
                headers.pop('x-opencode-directory', None)
                headers.pop('x-opencode-workspace', None)
            try:
                ...
                raise HTTPException(status_code=502, detail=f"OpenCode upstream failed: {exc}") from exc

            if request.method == "POST" and (native_path.rstrip("/") in {"session", "api/session"} or (bindings and route and native_path.endswith('/fork'))) and 200 <= status < 300:
                created_session_id = await _record_created_session(
                    registry, endpoint, agent_id, username, metadata, content, session_recorder,
                    v2=native_path.rstrip("/") == "api/session",
                )
                activity_persisted = True
                bind_request(request.scope, session_id=created_session_id)
                if bindings:
                    bound = await bindings.ensure(endpoint, registry.get_session_route(created_session_id))
            ...
            elif request.method == "POST" and native_path.endswith("/message") and 200 <= status < 300:
                try:
                    info = json.loads(content).get("info", {})
                    bind_request(request.scope, message_id=info.get("parentID"))
                except (ValueError, TypeError, AttributeError):
                    pass
            if management and request.method == 'GET' and native_path in {'config', 'global/config'}:
                from app.management import redact
                try:
                    content = json.dumps(redact(json.loads(content))).encode()
                    response_headers.pop('content-encoding', None)
                except (ValueError, TypeError):
                    raise HTTPException(502, 'Could not read managed runtime configuration')
    ...
    *, flatten: bool = False,
    ...
    try:
        ...
        if disconnected in done:
            raise HTTPException(status_code=499, detail="SSE client disconnected before upstream headers")
    ...
    async def chunks():
        try:
            async for chunk in (flattened_events(upstream) if flatten else upstream.aiter_raw()):
                yield chunk
        finally:
            await cleanup()
    ...
    return _TrackedStreamingResponse(
        ...
        media_type=None, cleanup=cleanup,
    )
...
def _extract_metadata(request: Request, body: bytes) -> tuple[RouteMetadata, bytes]:
    ...
    if request.method not in {"POST", "PUT", "PATCH"} or content_type != "application/json" or not body:
    ...
    if header_metadata.request_id and metadata.request_id and header_metadata.request_id != metadata.request_id:
        raise HTTPException(status_code=409, detail="conflicting request IDs")
    if header_metadata.agent_id and metadata.agent_id and header_metadata.agent_id != metadata.agent_id:
        raise HTTPException(status_code=409, detail="conflicting Agent routing metadata")
    if header_metadata.username and metadata.username and header_metadata.username != metadata.username:
...
def _optional_string(mapping: dict[str, Any], key: str) -> str | None:
    ...
    if value is not None and (not isinstance(value, str) or not value):
        raise HTTPException(status_code=400, detail=f"_cloud.{key} must be a non-empty string")
    return value
...
def _resolve_owner(route: SessionRoute | None, metadata: RouteMetadata) -> tuple[str, str]:
    if route is not None:
        if metadata.agent_id and metadata.agent_id != route.agent_id:
            raise HTTPException(status_code=409, detail="session Agent route conflicts with request")
        if metadata.username and metadata.username != route.username:
            raise HTTPException(status_code=409, detail="session user route conflicts with request")
        return route.agent_id, route.username
    if not metadata.agent_id or not metadata.username:
        raise HTTPException(status_code=400, detail="Agent and username routing metadata are required")
...
async def _record_created_session(
    ...
    *, v2: bool = False,
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="OpenCode session response lacks an ID") from exc
    if not isinstance(session_id, str) or not session_id:
        raise HTTPException(status_code=502, detail="OpenCode session response has an invalid ID")
