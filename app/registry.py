...
class Registry:
    def __init__(self, path: str | Path, migration_path: str | Path | None = None, *, synchronous: str = "FULL") -> None:
        ...
        self.migration_path = Path(migration_path or Path(__file__).resolve().parents[1] / "migrations/001_initial.sql")
    ...
    def upsert_agent(self, agent_id: str, config_path: str, image: str, enabled: bool = True) -> None:
        ...
        with self.connect() as connection:
            current = connection.execute(
                "SELECT config_path, image, enabled FROM agents WHERE agent_id=?", (agent_id,)
            ).fetchone()
    ...
    def get_sandbox(self, agent_id: str, username: str) -> SandboxRecord | None:
        with self.connect() as connection:
            row = connection.execute(
                "SELECT * FROM sandboxes WHERE agent_id=? AND username=?", (agent_id, username)
            ).fetchone()
        return self._sandbox(row)

    def upsert_sandbox(
        self, *, sandbox_id: str, agent_id: str, username: str,
        container_id: str | None, host_port: int | None, status: str,
        image_version: str, last_active_at: str | None = None,
    ) -> SandboxRecord:
    ...
    def list_sandboxes(self) -> list[SandboxRecord]:
        with self.connect() as connection:
            return [SandboxRecord(**dict(row)) for row in connection.execute("SELECT * FROM sandboxes")]
    ...
    def touch_sandbox(self, sandbox_id: str) -> None:
        with self._write_connection() as connection:
            ...
            connection.execute("UPDATE sandboxes SET last_active_at=? WHERE sandbox_id=?", (utc_now(), sandbox_id))

    def set_health_status(self, sandbox_id: str, status: str) -> None:
        """Health observation must not reset the idle clock."""
        with self._write_connection() as connection:
            connection.execute("UPDATE sandboxes SET status=? WHERE sandbox_id=?", (status, sandbox_id))

    def record_session(
        self, session_id: str, sandbox_id: str, agent_id: str,
        username: str, workspace_relpath: str,
    ) -> SessionRoute:
    ...
    def record_sessions_batch(
        ...
        with self._write_connection() as connection:
            ...
            for index, (session_id, sandbox_id, agent_id, username, workspace_relpath) in enumerate(entries):
                savepoint = f"session_entry_{index}"
                connection.execute(f"SAVEPOINT {savepoint}")
                try:
                    owner = connection.execute(
                        "SELECT agent_id, username FROM sandboxes WHERE sandbox_id=?", (sandbox_id,)
                    ).fetchone()
                    if owner is None:
                    ...
                    row = connection.execute(
                        """INSERT INTO sessions(
                           ...
                           ON CONFLICT(session_id) DO UPDATE SET
                             last_active_at=excluded.last_active_at
                           RETURNING *""",
                        (session_id, sandbox_id, agent_id, username, workspace_relpath, now, now),
                    ).fetchone()
                    route = self._session(row)
                    assert route is not None
                    if (route.sandbox_id, route.agent_id, route.username, route.workspace_relpath) != (
                        sandbox_id, agent_id, username, workspace_relpath
                    ):
                        raise ValueError("Session ID is already bound to another route")
                    connection.execute(
                        "UPDATE sandboxes SET last_active_at=? WHERE sandbox_id=?", (utc_now(), sandbox_id)
                    )
    ...
    def list_sessions_for_sandbox(self, sandbox_id: str) -> list[SessionRoute]:
        with self.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM sessions WHERE sandbox_id=? ORDER BY created_at", (sandbox_id,)
            ).fetchall()
    ...
    def reconcile(self, observed: Iterable[SandboxRecord]) -> list[str]:
        ...
        for record in records:
            resolved = self.upsert_sandbox(
                sandbox_id=record.sandbox_id, agent_id=record.agent_id,
                username=record.username, container_id=record.container_id,
                host_port=record.host_port, status=record.status,
                image_version=record.image_version, last_active_at=record.last_active_at,
            )
            observed_ids.add(resolved.sandbox_id)
        with self._write_connection() as connection:
            rows = connection.execute("SELECT sandbox_id FROM sandboxes").fetchall()
            missing = [row["sandbox_id"] for row in rows if row["sandbox_id"] not in observed_ids]
