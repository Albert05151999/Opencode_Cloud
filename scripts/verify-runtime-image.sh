...
image=${RUNTIME_IMAGE:-cloud-agent-runtime:dev}
...
cp "$project_dir/runtime/opencode/global/opencode.json" "$test_root/agent/opencode.json"
chmod -R a+rwX "$test_root/workspace" "$test_root/state"
chmod -R a+rX "$test_root/agent"
build_args=()
build_args+=(--build-arg "NODE_VERSION=$NODE_VERSION" --build-arg "OPENCODE_VERSION=$OPENCODE_VERSION" --build-arg "NODE_ARCHIVE_SHA256=$NODE_ARCHIVE_SHA256" --build-arg "UBUNTU_SNAPSHOT=$UBUNTU_SNAPSHOT")
case "${HTTPS_PROXY:-${https_proxy:-}}" in
  http://127.0.0.1:*) build_args+=(--network host) ;;
esac
for var in HTTP_PROXY HTTPS_PROXY NO_PROXY http_proxy https_proxy no_proxy; do
  test -z "${!var:-}" || build_args+=(--build-arg "$var=${!var}")
done
if [[ ${SKIP_BUILD:-0} != 1 ]]; then
  docker build "${build_args[@]}" -f "$project_dir/runtime/Dockerfile" -t "$image" "$project_dir"
fi
