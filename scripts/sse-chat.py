...
BASE_URL = "http://106.52.221.61:18080"
AGENT = "agent-code"
USERNAME = "local-demo"
...
class Display:
    ...
    def accept(self, event):
        ...
        if kind == "session.error":
            raise RuntimeError("Session error: " + json.dumps(props.get("error"), ensure_ascii=False))
        if kind == "message.updated" and info.get("role") == "assistant":
            self.assistants.add(info["id"])
            if info.get("error"):
                raise RuntimeError("Model error: " + json.dumps(info["error"], ensure_ascii=False))
        ...
        if kind == "session.idle" and self.busy:
            return True
        if kind == "message.part.delta" and props.get("messageID") in self.assistants and props.get("field") == "text":
            key = props["partID"]
...
async def chat(args):
    ...
    if not token:
        try:
            import keyring
            token = keyring.get_password('opencode-cloud-web', args.base_url.rstrip('/')) or ''
        except Exception:
            pass
    auth_headers = {'Authorization': 'Bearer ' + token} if token else {}
    async with httpx.AsyncClient(base_url=args.base_url.rstrip('/'), trust_env=False,
                                headers=auth_headers, timeout=httpx.Timeout(60, connect=10)) as client:
        created = await client.post('/session', json={'_cloud': {'agent_id': args.agent, 'username': args.username}})
        ...
        try:
            async with asyncio.timeout(args.timeout):
                async with client.stream('GET', '/event', headers={**headers, 'Accept': 'text/event-stream'},
                                         timeout=httpx.Timeout(None, connect=10)) as stream:
                    ...
                    submitted = True
                    response = await client.post(f'/session/{session}/prompt_async', json={
                        'messageID': 'msg_' + uuid.uuid4().hex,
                        'model': {'providerID': 'cloud-model-gateway', 'modelID': args.model},
                    ...
                    async for event in iterator:
                        if display.accept(event):
                            ...
                            return
                    raise RuntimeError('SSE disconnected before completion; request is not resent automatically')
        finally:
            if submitted and not completed:
                # Abort only the new session created by this invocation.
                try:
                    response = await client.post(f'/session/{session}/abort', timeout=10)
                    response.raise_for_status()
                    print('\nAbort requested for this session.', file=sys.stderr)
                except Exception:
                    print(f'Abort unconfirmed; check session {session} on the server.', file=sys.stderr)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--base-url', default=BASE_URL)
    parser.add_argument('--token-file', help='Administrator token file; alternatively use CLOUD_AGENT_ADMIN_TOKEN or the Web credential vault')
    parser.add_argument('--agent', default=AGENT)
    parser.add_argument('--username', default=USERNAME)
    parser.add_argument('--model', default=MODEL)
    parser.add_argument('--prompt', default=PROMPT)
    parser.add_argument('--timeout', type=float, default=TIMEOUT_SECONDS)
    parser.add_argument('--raw', action='store_true', default=RAW_EVENTS, help='Print original session SSE JSON instead of text')
    ...
    try:
        ...
        print(f'无法连接 {args.base_url}。请检查服务器地址、API 监听端口及网络访问规则。', file=sys.stderr)
        return 1
