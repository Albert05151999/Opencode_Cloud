    ...
    with tempfile.TemporaryDirectory(prefix='cloud-stage28-') as tmp:
        ...
        try:
            for index in range(21):
                await asyncio.gather(*(sample(agent, user, 'cold' if index == 0 else 'warm') for agent in ['agent-code', 'agent-data'] for user in ['profile-alice', 'profile-bob'] for _ in range(2)))
            report['result'] = 'passed'
        except Exception as exc:
            report.update(result='failed', error=type(exc).__name__)
            raise
        finally:
            (output / 'acquire-profile.json').write_text(json.dumps(report, indent=2) + '\n')
