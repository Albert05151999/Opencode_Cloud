...
def _routing_extension() -> dict[str, Any]:
    return {
        ...
        "session_directory_binding": {
            "since": "0.2.1",
            "directory": "/workspace/sessions/<sessionID>",
...
def _build_document(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            f"Pinned OpenCode API document is unavailable: {path}"
        ) from exc
    try:
    ...
    if (
        ...
        or source.get("openapi") != "3.1.0"
        or not isinstance(source.get("paths"), dict)
    ):
        raise RuntimeError(
            f"Pinned OpenCode API document has an unexpected structure: {path}"
        )
